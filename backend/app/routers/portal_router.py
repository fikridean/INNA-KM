...
@router.post(
    "/create",
    response_model=PortalCreateResponseModel,
...
# Get portals
@router.post(
    "/get", response_model=PortalGetResponseModel, status_code=StatusCode.OK.value
)
async def get_portals_route_func(params: PortalGetModel):
    try:
        data: List[PortalGetResponseModelObject] = await get_portals(params)
        return success_response(
            data, message=ResponseMessage.OK_LIST.value, status_code=StatusCode.OK.value
        )
...
# Get portal detail
@router.get(
    "/detail", response_model=PortalDetailResponseModel, status_code=StatusCode.OK.value
)
...
# Delete portal
@router.delete(
    "/delete", response_model=PortalDeleteResponseModel, status_code=StatusCode.OK.value
)
...
@router.get(
    "/retrieve-data",
    response_model=PortalRetrieveDataResponseModel,
    status_code=StatusCode.OK.value,
)
